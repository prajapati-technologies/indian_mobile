...
class _ProjectEditorScreenState extends State<ProjectEditorScreen> {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          GestureDetector(
            onPanStart: (details) {
              if (_selectedToolIndex == 0) provider.startDrawingWall(details.localPosition);
              else if (_selectedToolIndex == 3 && provider.selectedObject != null) {
              } else {
                final hit = _hitTestObject(provider, details.localPosition);
                if (hit == null) provider.selectObject(details.localPosition);
              }
            },
            onPanUpdate: (details) {
              if (_selectedToolIndex == 0) provider.updateDrawingWall(details.localPosition);
              else if (provider.selectedObject != null) provider.moveSelected(details.delta);
            },
            onPanEnd: (details) {
            ...
            onTapDown: (details) {
              if (_selectedToolIndex == 1) {
                provider.addOpening(OpeningType.door, details.localPosition);
              } else if (_selectedToolIndex == 2) {
                provider.addOpening(OpeningType.window, details.localPosition);
              } else if (_selectedToolIndex == -1) {
                provider.selectObject(details.localPosition);
              }
            },
            child: RepaintBoundary(key: _canvasKey, child: InteractiveViewer(
              minScale: 0.5,
              maxScale: 5.0,
              child: CustomPaint(
                size: Size.infinite,
          ...
          if (provider.selectedObject != null)
            Positioned(
              top: 100,
              left: 20,
              child: _objectEditPopup(provider),
            ),
  ...
  Widget _toolDeselectItem(CanvasProvider provider) {
    return GestureDetector(
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ListView(
                ...
                children: [
                  _furnitureItem('Sofa', Icons.weekend, () => provider.addFurniture('Sofa', position: _randomPosition())),
                  _furnitureItem('Bed', Icons.bed, () => provider.addFurniture('Bed', position: _randomPosition())),
                  _furnitureItem('Table', Icons.table_restaurant, () => provider.addFurniture('Table', position: _randomPosition())),
                  _furnitureItem('Chair', Icons.chair, () => provider.addFurniture('Chair', position: _randomPosition())),
                  _furnitureItem('Wardrobe', Icons.shelves, () => provider.addFurniture('Wardrobe', position: _randomPosition())),
                  _furnitureItem('Desk', Icons.desk, () => provider.addFurniture('Desk', position: _randomPosition())),
                ],
              ),
            ),
          ],
  ...
  Offset _randomPosition() {
    return Offset(
      100 + (DateTime.now().microsecondsSinceEpoch % 200).toDouble(),
      100 + (DateTime.now().microsecondsSinceEpoch ~/ 1000 % 200).toDouble(),
    );
  }
  ...
  Widget _objectEditPopup(CanvasProvider provider) {
    ...
    if (obj is WallModel) {
      final dist = (obj.start - obj.end).distance / 20;
      return Container(
        width: 220,
        ...
        child: Column(
          ...
          children: [
            ...
            _propRow('Length', '${dist.toStringAsFixed(1)}m'),
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                IconButton(
                  ...
                  tooltip: 'Rotate Left',
                  onPressed: () => provider.rotateSelectedFurniture(-0.785),
                ),
                IconButton(
            ...
            const SizedBox(height: 8),
            Center(
              child: ElevatedButton(onPressed: () => provider.selectObject(const Offset(-9999, -9999)), child: const Text('Done')),
            ),
          ],
        ),
      );
          ...
          children: [
            ...
            Wrap(
              ...
              children: colors.map((c) => GestureDetector(
                onTap: () {
                  provider.applyTextureToSelected(c.$2);
                  Navigator.pop(context);
                },
                child: Column(
                  children: [
  ...
  Future<void> _exportAsPng(CanvasProvider provider) async {
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Design exported as PNG'), behavior: SnackBarBehavior.floating),
        );
      }
    } catch (e) {
      if (mounted) {
